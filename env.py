...
class ConstructEnv:
    ...
    每步生成一个节点（四面体）加入图。
    Step 0（初始化）: 空图 → 神经网络选择初始四面体类型（0-4） → 放置 4 个粒子。
    Step 1+:         当前图 → 神经网络选择候选（开放面 + 新粒子类型） → 放置 1 个粒子。
    ...
    观测格式:
        ...
        cand_obs = {
            'face_types':    (N_cands,) int,  # 0-3，或 4（无面，step 0）
            'new_tet_types': (N_cands,) int,  # 0-4
            'n_cands': int,
        }
    """
    ...
    def _step_init(self, tet_type):
        """
        tet_type ∈ {0,1,2,3,4} = 大球数量。
        放置 4 个粒子构成初始四面体，类型由 tet_type 决定。
        """
        cfg = self.cfg
        n_L = tet_type
        n_S = 4 - n_L
    ...
    def _step_normal(self, action_idx):
        ...
        for m in range(n_new):
            ...
            gap  = dist - (all_rad[m] + all_rad[n_new])
            if gap < cfg.collision_tol:
                touching.append(m)
                self.contact_pairs.add((min(m, n_new), max(m, n_new)))

        # 找出所有以 n_new 为顶点的新四面体：
        # (a,b,c,n_new) 成立当且仅当 a,b,c 都与 n_new 接触，且 a,b,c 两两接触。
    ...
    def _add_tet(self, i, j, k, l):
        """将四面体 (i,j,k,l) 加入图，并与共享面的已有四面体连边。"""
    ...
    def _update_current_candidates(self):
        """将候选集合整理为有序列表（按配位数降序），构建 current_candidates。"""
        cands = []
        for cid, feat in self._candidate_set.items():
